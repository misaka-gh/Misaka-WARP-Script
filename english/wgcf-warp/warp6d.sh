...
generate_wgcf_config(){
    ...
    sed -i "7 s/^/PostUp = ip -6 rule add from $(ip route get 2606:4700:4700::1001 | grep -oP 'src \K\S+') lookup main\n/" wgcf-profile.conf
    sed -i "8 s/^/PostDown = ip -6 rule delete from $(ip route get 2606:4700:4700::1001 | grep -oP 'src \K\S+') lookup main\n/" wgcf-profile.conf
